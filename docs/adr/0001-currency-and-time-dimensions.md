...
ここでは `CostComponent.currency` は参照できない。componenet 単位で通貨が
...
pub struct PricedValue {
    pub value: f64,
    pub currency: String,   // runtime tag (例: "USD", "CNY")
}
...
pub trait TypedPricingProvider<C: CurrencyCode>: Send + Sync {
    fn lookup(&self, sku: &Sku) -> Result<Currency<f64, C>, PricingError>;
}

// ── 各 provider crate は両方を impl ──────────────────────────────
impl TypedPricingProvider<USD> for AwsPricingRegistry {
    fn lookup(&self, sku: &Sku) -> Result<Currency<f64, USD>, PricingError> {
        ...
        Ok(Currency::new(entry.value))
    }
}

impl PriceCatalog for AwsPricingRegistry {
    fn lookup(&self, sku: &Sku) -> Result<PricedValue, PricingError> {
        // Tier B を呼び出し、Currency<f64, USD> → PricedValue へ erase
        let typed: Currency<f64, USD> =
            <Self as TypedPricingProvider<USD>>::lookup(self, sku)?;
        Ok(typed.erase())   // C::CODE.to_string() で String に変換
    }
}
```
...
- `yevice-pricing` に `Currency<T, C>` / `Money` / `BillingPeriod` /
  `ExchangeRates` / `PricedValue { value: f64, currency: String }` を新設。
- 通貨型を two-tier trait で表現する:
  - `PriceCatalog` (既存、dyn-friendly 維持): associated type を持たず、
    `lookup(&self, sku) -> Result<PricedValue, PricingError>` を返す。
  ...
  - `TypedPricingProvider<C: CurrencyCode>` (新規、generic): 各 provider crate
    内部で `Currency<f64, C>` を返す静的型安全 trait。dyn 化はしない。
  - 各 provider は両 trait を impl し、`PriceCatalog::lookup` の内部で
    `TypedPricingProvider::lookup` を呼んで `Currency<f64, C>.erase()` で
    `PricedValue` に変換する (erase 境界 = provider crate の dyn surface 越境)。
