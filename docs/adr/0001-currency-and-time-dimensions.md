...
#### ResourceCost 構築時の Validation 契約

優先順位ルール (`CostComponent.currency > ResourceCost.currency > PricingMetadata
> USD fallback`) は **components 評価成功経路** ではすべての値の通貨を解決できる。
ところが `evaluate.rs` には **`ResourceCost.expr` への fallback 経路** (components
が空、または components 評価失敗時に親 `expr` を直接評価する経路) があり、
ここでは `CostComponent.currency` は参照できない。component 単位で通貨が
オーバーライドされていた場合、fallback で `ResourceCost.currency` を採用すると
component-level の通貨情報が静かに失われる。

これを防ぐため `ResourceCost::new(...)` (および serde deserialize 後の
post-validation hook) で以下を validate する:

- **すべての `CostComponent.currency` (Some の場合) と `ResourceCost.currency`
  (Some の場合) が一致すること、もしくはすべて None であること**
- 不一致なら `CostBuildError::ComponentCurrencyMismatch { resource_id,
  currencies: Vec<String> }` で構築を reject する
...
pub enum CostBuildError {
    ...
    ComponentCurrencyMismatch {
        resource_id: String,
        currencies: Vec<String>,   // 検出された全 currency 値 (dedup 済み)
    },
}
```

この validation により以下が成立する:

- **ResourceCost レベルで通貨が一意に確定する** — 構築済み `ResourceCost` は
  「すべての component が同一通貨」または「全 component が currency None」の
  どちらかであることが保証される。
- **`ResourceCost.expr` fallback 経路でも通貨が失われない** —
  `ResourceCost.currency` (またはどの component の `currency` でも同じ値) から
  正しく通貨を取得できる。
- **評価時の優先順位ルールはそのまま** — `CostComponent.currency >
  ResourceCost.currency > PricingMetadata > USD fallback` を維持できる。mismatch
  は構築時に弾かれるため、評価時に矛盾が発生する余地がない。

##### スコープと将来拡張

「サービス実装が同一リソース内で混在通貨を出力したい」というケースは現状
ゼロ件 (Phase 1 では `TypedPricingProvider<USD>` のみ実装、AWS 中国リージョン
等は別 provider として分離する方針)。将来このケースが必要になった場合の
選択肢:

1. **ResourceCost を分割** — 通貨ごとに別の `ResourceCost` として表現する。
   `Architecture` レベルでは `BTreeMap<String, f64>` (`totals_by_currency`) で
   集約されるため、分割しても合計表示は変わらない。
2. **混在を許容するように本契約を緩める** — ADR 拡張として別途記録し、
   評価時の fallback 経路で `ResourceCost.expr` 全体を単一通貨に縛らない
   設計を検討する (e.g. expr 内に通貨タグを埋め込む、components を必須化する)。

現時点ではユースケースがないため案 1 を将来の暫定方針とし、ResourceCost
構築時に mismatch を hard error として弾く。
...
#### 制約: `PriceCatalog` は dyn-friendly でなければならない

yevice には既存の dyn 化された `PriceCatalog` trait があり、
`ProviderPlugin::pricing_catalog() -> Box<dyn PriceCatalog>` および
`MultiProviderCatalog: HashMap<Provider, Box<dyn PriceCatalog>>` が
provider-neutral な resolver の中核を成す。`PriceCatalog` に associated type
`Currency: CurrencyCode` を載せると Self の Currency が単一に確定しないため
trait object 化できなくなり、`MultiProviderCatalog` の動的構成が崩壊する。

このため通貨を **「dyn 表面 (object-safe)」** と **「provider 内部の静的型安全」**
の二段構えで表現する。

**採用: Two-tier trait + runtime mismatch reject**

既存の `PriceRecord` (Scalar / Tiered の二バリアント) は維持し、currency を
record 全体に **1 つ** 付与する形で拡張する。Tiered 内のすべての tier は
同じ通貨であることが前提（同一 Tiered record 内での通貨混在は型で禁止）。

```rust
// ── Tier A: dyn-friendly surface (yevice-pricing-core) ──────────
// associated type を持たず、Self::Currency に依存しない。
// 戻り値は値レベルで通貨タグを持つ erased 型。
pub trait PriceCatalog: Send + Sync {
    fn lookup(&self, sku: &Sku) -> Result<PricedValue, PricingError>;
}

// `PricedValue` は既存 `PriceRecord` (Scalar/Tiered) に currency を付けた erased 形。
// scalar / tiered のどちらのケースも保持し、サービス側の `.as_tiered()` 経路
// (S3 storage tiers, data transfer egress tiers, GuardDuty volume tiers 等) を
// 維持する。
pub enum PricedValue {
    Scalar {
        value: f64,
        currency: String,        // runtime tag (例: "USD", "CNY")
    },
    Tiered {
        tiers: Vec<PricedTier>,
        currency: String,        // record 単位で 1 つ — tier 間で通貨は混在しない
    },
}
...
// ── Tier B: provider crate 内部の静的型安全 trait ─────────────
// generic / associated type を持つ。dyn 化はしない。
// 戻り値も Scalar/Tiered を型レベルで保持する。
...
// ── 各 provider crate は両方を impl ──────────────────────────────
impl TypedPricingProvider<USD> for AwsPricingRegistry {
    fn lookup(&self, sku: &Sku) -> Result<TypedPriceRecord<USD>, PricingError> {
        let entry = self.find_entry(sku)?;
        // Bulk API の metadata.currency と C::CODE を照合
        ...
        Ok(entry.into_typed_record::<USD>())   // Scalar / Tiered を保持
    }
}

impl PriceCatalog for AwsPricingRegistry {
    fn lookup(&self, sku: &Sku) -> Result<PricedValue, PricingError> {
        // Tier B を呼び出し、TypedPriceRecord<USD> → PricedValue へ erase
        let typed: TypedPriceRecord<USD> =
            <Self as TypedPricingProvider<USD>>::lookup(self, sku)?;
        Ok(typed.erase())   // バリアントを保ったまま C::CODE.to_string() に変換
    }
}
```

`TypedPriceRecord::erase()` のセマンティクス:

```text
TypedPriceRecord::Scalar(Currency<f64, C>)
    → PricedValue::Scalar { value, currency: C::CODE.to_string() }

TypedPriceRecord::Tiered(Vec<TypedTier<C>>)
    → PricedValue::Tiered {
          tiers: Vec<PricedTier>,           // 通貨情報は record 側に寄せる
          currency: C::CODE.to_string(),
      }
```

サービス側 (`crates/services/yevice-services-aws/src/services/*.rs`) は
`PricedValue::Tiered { tiers, .. }` から既存と同じく `Vec<Tier>` 相当を取り出して
`Expr::tiered(...)` を構築する。`.as_tiered()` ヘルパーは `&[PricedTier]` を返す
形で残せる (currency は別チャネルで取り出す)。
...
- Provider crate 内部で `TypedPricingProvider<C>::lookup` を呼ぶコードパスは
  `Currency<f64, C>` を保持する (compile-time に通貨が型レベルで乗る)。
- Provider crate の dyn 表面 (`impl PriceCatalog for ...`) を越境するときに
  `.erase()` で `PricedValue { value, currency: String }` に変換する。
  これが Layer 1 の終点であり、ここを越えると runtime tag に切り替わる。
- `MultiProviderCatalog: HashMap<Provider, Box<dyn PriceCatalog>>` は従来通り
  動作し、provider-neutral な resolver は何も変更されない。

**得られる保証:**

- **Compile time** (provider crate 内部): 戻り値 `Currency<f64, C>` に通貨が
  型レベルで乗る (Phantom 維持)。同一 provider 内で複数 SKU を集約する際の
  通貨ミスマッチも型エラーになる。
- **Runtime** (`PriceCatalog::lookup` 経由): Bulk API ファイルの実 metadata と
  provider 宣言通貨の不一致は明示的エラーとして即座に表面化する。silent
  mislabeling は起こらない。`PricingError::CurrencyMismatch` は
  `PricingError` のバリアントとして dyn surface にも伝播する。
- 中国リージョン (`cn-north-1`) の CNY 価格を AWS グローバル provider
  (`TypedPricingProvider<USD>`) で読み込むと `CurrencyMismatch` で即 error し、
  ユーザーが認知可能。
- **`Box<dyn PriceCatalog>` は維持される** ため `ProviderPlugin::pricing_catalog`
  と `MultiProviderCatalog` の動的構成は影響を受けない。
...
1. **provider 分割 (推奨)**: `AwsUsdRegistry` (`TypedPricingProvider<USD>`) と
   `AwsChinaCnyRegistry` (`TypedPricingProvider<CNY>`) に分ける。型安全性を
   維持しつつ region 境界で provider を選択する。AWS 中国リージョンは IAM /
   API endpoint も別系統 (`amazonaws.com.cn`) であり、provider を分けるのが
   自然。両者とも `PriceCatalog` を impl するため `MultiProviderCatalog` には
   別エントリとして登録できる。
2. **enum-based provider**: `enum AwsRegistry { Usd(AwsUsdRegistry),
   Cny(AwsChinaCnyRegistry) }` で runtime dispatch。型レベルの通貨情報は
   失われるが API 表面は単一に保てる。
3. **`TypedPricingProvider` を複数 impl**: 同一 struct に `TypedPricingProvider<USD>`
   と `TypedPricingProvider<CNY>` の両方を impl する。Tier B は generic trait
   なので問題なく成立するが、Tier A (`PriceCatalog`) では SKU から実行時に
   どちらを呼ぶか分岐するロジックが必要になる。

**当面は案 1 (provider 分割) を推奨**。最も型安全で、現状 AWS 中国リージョンは
別 provider として扱う方が API endpoint / 認証経路の観点でも自然。Phase 1 では
AWS グローバル (`TypedPricingProvider<USD>` + `PriceCatalog`) のみ実装し、
中国リージョン等は将来別 provider として追加する。

## Consequences

- `yevice-pricing` に `Currency<T, C>` / `Money` / `BillingPeriod` /
  `ExchangeRates` / `PricedValue` (Scalar/Tiered enum, currency 付き) /
  `PricedTier { upper_limit, unit_price }` を新設。既存の `PriceRecord`
  (Scalar/Tiered) を currency 付きに拡張する形で、サービス側の `.as_tiered()`
  経路 (S3 storage tiers, data transfer egress tiers, GuardDuty volume tiers 等)
  を維持する。
- 通貨型を two-tier trait で表現する:
  - `PriceCatalog` (既存、dyn-friendly 維持): associated type を持たず、
    `lookup(&self, sku) -> Result<PricedValue, PricingError>` を返す。戻り値は
    Scalar/Tiered を保持した erased enum (currency: String を record 単位で持つ)。
    `Box<dyn PriceCatalog>` / `MultiProviderCatalog` / `ProviderPlugin::pricing_catalog`
    の動的構成は影響を受けない。
  - `TypedPricingProvider<C: CurrencyCode>` (新規、generic): 各 provider crate
    内部で `TypedPriceRecord<C>` (Scalar/Tiered enum、各値は `Currency<f64, C>`)
    を返す静的型安全 trait。dyn 化はしない。
  - 各 provider は両 trait を impl し、`PriceCatalog::lookup` の内部で
    `TypedPricingProvider::lookup` を呼んで `TypedPriceRecord<C>.erase()` で
    `PricedValue` に変換する。バリアント (Scalar/Tiered) は erase 境界を
    跨いで保持される。erase 境界 = provider crate の dyn surface 越境。
  - 同一 Tiered record 内で通貨は **1 つに固定**。tier 間で通貨を混在させたい
    ユースケースは想定外 (現状の Bulk API も持たない)。
...
- `ResourceCost::new(...)` 構築時に validation 契約を追加: すべての
  `CostComponent.currency` (Some の場合) と `ResourceCost.currency` (Some の場合)
  が一致するか、全 None であることを検査し、不一致なら
  `CostBuildError::ComponentCurrencyMismatch { resource_id, currencies }` で
  reject する。これにより `ResourceCost.expr` への fallback 評価経路でも
  component-level 通貨情報が失われない。
