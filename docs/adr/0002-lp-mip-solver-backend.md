...
##### `OptimizationProblem.bindings` を経由する間接参照の扱い

**Classifier は `OptimizationProblem.bindings` を考慮する必要がある。** Ceil が
直接 objective / constraint 内に出現せず、`VariableBinding` (中間変数) 経由で
間接的に流れ込むケースがあるため。例:

```
bindings:    t = ceil(x)
constraint:  t >= 10        // syntactic に ceil を含まないが意味上は ceil(x) >= 10
```

このケースを bindings 展開なしで classifier に通すと、constraint 側で ceil の
出現を検出できず anti-tight 文脈 (Ge LHS 正係数) を素通りさせ、上記の reject
規則を回避してしまう。

採用する判定方式は **bindings 展開ベース** (推奨):

1. **Bindings 展開フェーズ**: `problem.bindings` を substitution し、objective と
   constraints の全 expr を bindings なしの正規形に変換する。例の constraint
   `t >= 10` は `ceil(x) >= 10` に展開される。展開は固定点に達するまで反復し、
   `validate_bindings` が事前に循環を検出している前提で進める。
2. **正規形に対する context check**: 展開後の objective と constraints について、
   ceil の出現位置 × 係数符号 × constraint sense を判定する。判定ロジックは
   syntactic な場合と同一 (許可: minimization objective 正係数 / Le LHS 正係数
   / Ge LHS 負係数。Reject: 他)。

代替案として「bindings の expr 内に ceil がある場合、その binding の使用先を
逆引きして context を判定する」アプローチも考えうるが、展開ベースの方が既存の
syntactic 判定ロジックをそのまま再利用でき、binding が他の binding に渡される
ような連鎖ケースも自然に扱える。

#### 実装時の前提検出義務

solve 冒頭で **`validate_bindings(problem)` を先に走らせて** binding の循環や
未定義参照を検出した上で、`expr_introspect::classify_ceil_context(problem)` を
走らせる。classifier は内部で次の順に処理する:

0. **Bindings 展開**: `problem.bindings` を substitution して objective と
   constraints の全 expr を正規形 (bindings なしの形) に変換する。
   `validate_bindings` 通過済みなので循環は存在しない。
1. 展開後の各 ceil について **出現箇所** を特定 (objective / constraint index)
2. 係数符号 (正 / 負)
...
`expr` フィールドには **展開後の正規形** (e.g. `ceil(x)`) を文字列化して載せる。
これにより binding 経由で間接的に流れ込んだケースでも、ユーザーは reject 対象の
ceil 式を直接確認できる。
